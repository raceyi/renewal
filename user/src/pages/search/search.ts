...
export class SearchPage {
  shops=[];
  visited=[];
  shopSelected:boolean=false;  // => 없애고 모두 loadingCtrl로 바꾸자. 화면 바꾸면 dismiss한다.
  ...
  goToShop(shop){
    ...
    console.log("[goToShop]takitId:"+takitId);
          ...
          if(!this.shopSelected){
            ...
            this.serverProvider.getShopInfo(takitId).then((res:any)=>{
                ...
                //}else{
                //    this.app.getRootNavs()[0].push(ShopEntrancePage,{takitId:takitId});
                //}
            },(err)=>{
