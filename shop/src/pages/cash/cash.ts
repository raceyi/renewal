...
export class CashPage {
  ...
  doWithraw(){
     //takitId, bankCode(은행코드 3자리) , withdrawalAmount(인출금액),fee
        let body = JSON.stringify({takitId:this.storageProvider.myshop.takitId,
                                ...
                                withdrawalAmount:this.storageProvider.withdrawable.netSales,//cashAvailable,
      ...
      this.serverProvider.post("/shop/withdrawCash",body).then((res:any)=>{
          ...
          if(res.result=="failure" && res.error=='invalid value'){
            let alert = this.alertController.create({
                title: '화면과 인출 가능 잔액이 일치하지 않습니다.',
                buttons: ['OK']
            });
            alert.present();
            return;
  ...
  withdrawCash(){
     ...
     if(this.storageProvider.withdrawable.netSales<=0){
            let alert = this.alertController.create({
