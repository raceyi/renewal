...
export class TimeUtil {
  ...
  getlocalDate(string){
    ...
    let dayStr=(day <= 9 ? '0': '') + (day);
    let fullYear:string=d.getFullYear().toString();
    let localDateString=fullYear.substr(2,2)+"/"+monthStr+"/"+dayStr;
    console.log("getlocalDate:"+localDateString);
    return localDateString;
  }


  getlocalTimeStringWithoutDate(string){
    let d=new Date(this.getGMTtimeInMilliseconds(string));
    //console.log("date:"+d.getDay());
    let hour=d.getHours();
    let hourStr=(hour <= 9 ? '0': '') + (hour);
    let min=d.getMinutes();
    let minStr=(min <= 9 ? '0': '') + (min);
    let localTimeString=hourStr+":"+minStr;
    console.log("getlocalTimeStringWithoutDay:"+localTimeString);
    return localTimeString;
  }

  getlocalTimeStringWithoutYear(string){
    let d=new Date(this.getGMTtimeInMilliseconds(string));
    //console.log("date:"+d.getDay());
    let month=d.getMonth()+1;
    let monthStr=(month <= 9 ? '0': '') + (month);
    ...
    let min=d.getMinutes();
    let minStr=(min <= 9 ? '0': '') + (min);
    let localTimeString=monthStr+"/"+dayStr+" "+hourStr+":"+minStr;
    console.log("getlocalTimeStringWithoutDay:"+localTimeString);
    return localTimeString;
  }
