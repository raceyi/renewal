...
export class ServerProvider{
  ...
  updateCashAvailable(){
      return new Promise((resolve,reject)=>{
           //let command="/shop/getBalance";
           let command="/shop/computeBalance";
           let body = JSON.stringify({takitId:this.storageProvider.myshop.takitId});
           console.log(command+body);
           this.post(command,body).then((res:any)=>{
                console.log("res:"+JSON.stringify(res));
                if(res.result=="success"){
                    this.storageProvider.withdrawable=res.sales.withdrawable;
                    this.storageProvider.withdrawable.netSales=parseInt(res.sales.withdrawable.sales)-parseInt(res.sales.withdrawable.fee);
                    this.storageProvider.available=res.sales.available;
                    resolve(res);
                }else{
